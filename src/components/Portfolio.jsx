...
// Sub-componente actualizado con estado para móviles
const ProjectCard = ({ project, t }) => {
  // 1. Agregamos un estado para controlar el toque en móvil
  const [isTapped, setIsTapped] = useState(false);

  return (
    <div
      // 2. Al hacer clic (tap), alternamos el estado.
      // Agregamos 'select-none' para evitar que el navegador intente seleccionar texto/imagen al tocar.
    ...
    >
      ...
      >
        {/* Título: Ajustamos el translate-y según el estado isTapped */}
        <h3
          className={`text-2xl font-bold transition-transform duration-300 
          ${isTapped ? 'translate-y-0' : 'translate-y-10'} group-hover:translate-y-0`}
        >
          {project.title}
        </h3>

        {/* Descripción: Ajustamos el translate-y según el estado isTapped */}
        <p
          className={`mt-2 text-sm transition-transform duration-300 delay-75 
          ${isTapped ? 'translate-y-0' : 'translate-y-10'} group-hover:translate-y-0`}
        >
        ...
        <div
          className={`flex flex-wrap justify-center gap-2 mt-4 transition-transform duration-300 delay-100 
          ${isTapped ? 'translate-y-0' : 'translate-y-10'} group-hover:translate-y-0`}
        >
          {project.techStack &&
        ...
        {/* Enlaces: Usamos stopPropagation para que al tocar el link no se cierre el overlay antes de navegar */}
        <div
          className={`flex gap-4 mt-6 transition-transform duration-300 delay-150 
          ${isTapped ? 'translate-y-0' : 'translate-y-10'} group-hover:translate-y-0`}
        >
          <a
